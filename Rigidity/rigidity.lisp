...
(defclass spring ()
  ((%horizontal-rigidity :initarg :horizontal-rigidity :accessor horizontal-rigidity)
   (%vertical-rigidity :initarg :vertical-rigidity :accessor vertical-rigidity)))

(defclass simple-spring (spring) ())

(defclass grid-spring (spring)
...
(defgeneric impose-width (spring width))
(defgeneric impose-height (spring height))

(defmethod impose-width ((spring simple-spring) width)
  (format t "imposing width ~a~%"  width))

(defmethod impose-height ((spring simple-spring) height)
  (format t "imposing height ~a~%"  height))

(defmethod impose-width ((spring grid-spring) width)
  (let ((grid (grid spring)))
    (let ((rows (array-dimension grid 0))
	  (cols (array-dimension grid 1)))
      (if (= cols 1)
	  (loop for row from 0 below rows
		do (impose-width (aref grid row 0) width))
	  (loop for individual-width in (sizes-in-series (combined-cols spring) width)
		for col from 0
		do (loop for row from 0 below rows
			 do (impose-width (aref grid row col) individual-width)))))))

(defmethod impose-height ((spring grid-spring) height)
  (let ((grid (grid spring)))
    (let ((rows (array-dimension grid 0))
	  (cols (array-dimension grid 1)))
      (if (= rows 1)
	  (loop for col from 0 below cols
		do (impose-height (aref grid 0 col) height))
	  (loop for individual-height in (sizes-in-series (combined-rows spring) height)
		for row from 0
		do (loop for col from 0 below cols
			 do (impose-height (aref grid row col) individual-height)))))))

(defun combine (grid-spring)
  (assert (typep grid-spring 'grid-spring))
  (let ((grid (grid grid-spring)))
    (assert (= (array-rank grid) 2))
    (let ((rows (array-dimension grid 0))
	  (cols (array-dimension grid 1)))
      (assert (or (> rows 1) (> cols 1)))
      (cond ((= rows 1)
	     (setf (combined-cols grid-spring)
		   (loop for col from 0 below cols
			 collect (horizontal-rigidity (aref grid 0 col))))
	     (setf (horizontal-rigidity grid-spring)
		   (combine-in-series (combined-cols grid-spring)))
	     (setf (vertical-rigidity grid-spring)
		   (combine-in-parallel
		    (loop for col from 0 below cols
			  collect (vertical-rigidity (aref grid 0 col))))))
	    ((= cols 1)
	     (setf (combined-rows grid-spring)
		   (loop for row from 0 below rows
			 collect (vertical-rigidity (aref grid 0 row))))
	     (setf (horizontal-rigidity grid-spring)
		   (combine-in-parallel
		    (loop for row from 0 below rows
			  collect (horizontal-rigidity (aref grid 0 row)))))
	     (setf (vertical-rigidity grid-spring)
		   (combine-in-series (combined-rows grid-spring))))
	    (t
	     (setf (combined-rows grid-spring)
		   (loop for row from 0 below rows
			 collect (combine-in-parallel
				  (loop for col from 0 below cols
					collect (vertical-rigidity (aref grid row col))))))
	     (setf (combined-cols grid-spring)
		   (loop for col from 0 below cols
			 collect (combine-in-parallel
				  (loop for row from 0 below rows
					collect (horizontal-rigidity (aref grid row col))))))
	     (setf (horizontal-rigidity grid-spring)
		   (combine-in-series (combined-cols grid)))
	     (setf (vertical-rigidity grid-spring)
		   (combine-in-series (combined-rows grid))))))))
