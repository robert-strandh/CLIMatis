...
\seesec{sec-zones-layout-vbox} except that the behavior is simper in
...
The children of the \code{vtree} zone are organized as a
self-adjusting \emph{tree}.  The immediate consequence of this
organization is that when the function \code{(setf vpos)} is used on
the \code{vtree} zone, it determines which children are likely to be
visible and move them close to the root of the tree.  The overall
effect allows for the \code{vtree} to have a very large number of
children, while still painting the visible children in an efficient
way. 

The \code{vtree} zone contains a tree of \emph{internal} zones that
can not be used in a context other than as children of the
\code{vtree} zone.\footnote{say more}.
