...
\code{visit} zone and the \code{motion} zone.  Input from the keyboard
and from pointer button is not treated by any zone, but is handled
globally for the application.  \seechap{chap-input-events}
...
Four generic functions are involved in input processing:
...
These four generic functions are called whenever the corresponding
input event is detected.  These functions take tree arguments:

\begin{itemize}
\item A \emph{handler}.
\item The \emph{code} of the key or button involved.
\item The \emph{modifier state} at the time the event happened.
\end{itemize}

The handler should be an instance of the class \code{key-handler} for
handling keyboard events and an instance of the class
\code{button-handler} for handling pointer button events.  The user
may create subclasses of these classes, and add methods on these
generic functions specialized for those subclasses. 
