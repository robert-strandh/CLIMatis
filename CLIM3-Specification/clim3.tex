...
\item An application should be able to resize a zone by setting its
  width and height.  The result of the operation should be immediately
  visible after the next iteration of the event loop.  Though of
  course, it is again possible that the zone being resized by the
  application is again resized by its parent zone, if the parent zone
  is a layout zone that imposes the size of its children.  In that
  case, there will be no visible effect.
\item an application should be able to modify the depth of a zone.  
\item An application should be able to modify the horizontal or the
  vertical gives of a zone.  When the give of the zone is assigned
  permanently, then the effect of this operation should be immediately
  visible after the next iteration of the event loop.  Though, again,
  if the give of the zone is a function of the give of its children,
  then this modification will be immediately undone, so that there is
  no visible effect.
...
\subsection{Resizing a zone}

If the zone is a root zone, we either ask the display server to resize
the corresponding window, or we do nothing, assuming that the window
manager has already decided the size of the corresponding window. 

If the zone is the child of bboard zone, then we need to render it
...
If the zone if part of a layout zone that imposes positions of the
child, then resizing it will have no effect. 
...
\subsection{Modifying the gives of a zone}

If the zone is the root zone, we might compute the natural size and
ask the display server to resize the corresponding window, or we might
do nothing.  

If the zone is the child of a bboard zone or a scroller zone, then a
new natural size must be determined, and then we do as if we resized
it.
...
\section{Gives invalidation protocol}

The gives invalidation protocol is divided into four functions
for maximum flexibility to client code, three of them generic, and one
